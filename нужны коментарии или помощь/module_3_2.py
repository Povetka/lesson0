...
# оно заработало но не так как надо, потому что мне явно нужно помочь разобраться с циклом фор.
    area = recipient, sender
    for i in area:
        if '@' or '.com' or '.ru' or '.net' in area:
            pass
        else:
...
# def send_email(message, recipient, *, sender="university.help@gmail.com"):
#     if '@' or '.com' or '.ru' or '.net' not in recipient or sender: # что не так с этой строчкой
#         # оно находит, что одно из условий не содержится в recipient or sender и переходит к принту
#         # но надо наоборот, чтобы когда оно находит хоть одно условие тогда переходило к следующему шагу
#         # как это сделать, я не понимаю.
#         # возможно тут нужен цикл фор, но я не понимаю как его тут использовать
#         print(f'"Невозможно отправить письмо с адреса {sender} на адрес {recipient}".')
