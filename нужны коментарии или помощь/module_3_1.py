...
def is_contains(string, list_to_search):
    ...
    for i in list_to_search:
        if string.lower() in i.lower() or i.lower() in string.lower():
            # все работает норм, но хочу уточнить, почему в строке выше ищем в i в нижнем регистре, а не в list_
            # я сделала этот код рабочим методом научного тыка, сама не пойму, почему это работает
            return True
        else:
            return False
